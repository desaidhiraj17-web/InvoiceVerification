...
#  ***************** Helpers Import  *******************
from src.helpers.invoices import check_invoice_exists,FlowType
...
async def match_scan_product(
    ...
    """ Matches scanned product details against the product master using batch, expiry, MRP, MFG, and barcode.
        Validates invoice existence and required search parameters before initiating the match process.
        Performs hierarchical search using direct batch match, fuzzy logic, and progressive filtering.
        Supports barcode-based direct lookup when available for high-accuracy matching.
        Returns a list of matched products along with total count and match status. """
...
async def get_products_batch_number(
    ...
    """ Searches products by batch number prefix with pagination support.
    First searches in Product Master and falls back to Invoice Product List if no results are found.
    Optionally uses invoice context for targeted fallback search when provided. """
...
async def get_racks(
    ...
    """ Fetches a paginated list of racks from the rack master.
        Returns total count along with page-wise rack data sorted by last updated time.
...
async def get_product_qty_converter_list(
    ...
    """ Fetches a paginated list of product quantity conversion mappings.
        Supports optional text search on product name and item code."""
...
async def update_product_qty_converter(
    ...
    """ Updates quantity conversion values (shipper, box, strip) for an existing product.
        Validates product existence and ensures at least one field is provided for update.
        Tracks updated timestamp and the user performing the update.
        Returns the updated product quantity mapping on success."""
...
async def get_invoice_no(
    ...
    """ Fetches the currently assigned invoice ID for a given tray number.
        Returns tray details along with the active invoice mapping. """
...
@router.put("/scan-quantity")
async def update_scanned_qty_products(data: ProductScanQtyUpdate, type: FlowType,db: AsyncSession = Depends(get_db), 
                        current_user: User = Depends(get_current_user)):
    
    """ Updates scanned quantities and scan status for multiple products under a given invoice.
        Inserts or updates product quantity converter values and supports concurrent processing.
        Optionally releases associated trays when the invoice is marked as completed. """
    ...
    try:
        ...
        async def limited_scan(product,type):
            async with sem:
                return await scan_quantity_update_products(db, data.invoice_id, product, type,current_user)
        tasks = [limited_scan(product,type) for product in data.products]
